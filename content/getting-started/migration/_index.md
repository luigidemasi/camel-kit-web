...
Migration questions appear one at a time as `[Concern N of M]` or `[Clarification N of M]` blocks: what was found, why it matters, and multiple-choice options when possible; open-ended questions are used when necessary. If a subagent encounters an unresolved connector choice or another guide step that asks you a question, it returns `NEEDS_CONTEXT` with every open decision and leaves dependent output unwritten. The parent conversation asks each question, records your answers as `✓ Confirmed` in the analysis summary, and resumes the same step with those answers. For example, an Anypoint MQ connector triggers a choice of replacement before that choice is written into `business-requirements.md`. `NEEDS_USER_CONFIRMATION` retains its separate role for content-derived actions described above.
