...
**Requirements and compatibility:** the first Ship worker runs on Linux only and needs Pi, Node, and Bubblewrap on a merged-/usr host. Harness compatibility is tiered: *supported* means a maintained configuration covered by a live end-to-end test, *experimental* means runnable but not covered by that gate, *incompatible* means a required capability is known absent, and *untested* means no current result. The maintained configuration is Pi 0.83.0 with Node 22.22.2; any other detected version is reported as experimental and runs only with `--accept-experimental`. The live end-to-end gate is a manual, maintainer-run authenticated test — not a CI default — and it has not yet been completed, so no configuration is currently covered by it.
