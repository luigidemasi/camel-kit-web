...
Runtime verification is handled internally by `camel-verify` during execution. `/camel-ship` is a [Technology Preview](../ship/#overview) that delegates to the local `camel-kit ship` controller, which runs its own workflow and never stores its run state in the manual pipeline file, while `/camel-knowledge` answers Camel documentation questions without changing pipeline state.
