...
Ship is a local workflow controller. The command `camel-kit ship` starts, inspects, resumes, or aborts a Ship run on your machine. The `0.4.1` Camel JBang plugin exposes the equivalent `camel kit ship` form. One run takes an integration from requirements to published code through five controller-owned stages: discovery, design, plan, execute, and validate.
