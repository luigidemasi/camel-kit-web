...
| **Knowledge MCP** | Hybrid semantic search | 20,367 indexed documents |
...
One deliberate exception: the Ship workflow, which is a **Technology Preview** and is not recommended for production use. See [preview status](../pipeline/ship/#overview). `/camel-ship` is a short delegate to the registered `camel-kit ship` command, and the workflow controller — stages, run state, oversight, evidence, and guarded publication — is compiled code in the Camel-Kit CLI, not a prompt. Eligible Bob 2, Copilot CLI and Claude Code sessions relay controller-issued tasks to read-only native subagents and submit their structured results. Ship retains candidate writes, validation and publication, and each run keeps its recorded backend. Copilot uses a synchronous custom-agent task and Claude Code a `camel-ship-worker` subagent with an enforced read-only tool allowlist; both inherit parent path permissions, and a fresh context is not filesystem isolation. See [native Ship execution](../pipeline/ship/#claude-code-native-subagents). Every other routed command remains prompt-owned.
